...
import { Star, SlidersHorizontal, X, Loader2, PackageOpen, ChevronLeft, ChevronRight } from "lucide-react";
...
function ProductsContent() {
  ...
  const { data: bannersData } = useGetBanners();
  ...
  const prevBanner = useCallback(() => {
    setBannerIndex((prev) => (prev - 1 + (banners.length || 1)) % (banners.length || 1));
  }, [banners.length]);
      ...
      {banners.length > 0 && (
        <div className="relative mb-6 w-full overflow-hidden rounded-2xl">
          <AnimatePresence mode="wait">
            ...
            >
              <Image
                src={banners[bannerIndex].image.url}
